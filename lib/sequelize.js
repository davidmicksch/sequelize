...
class Sequelize {
  ...
  sync(options) {
    ...
    return Promise.try(() => {
      ...
      return Promise.each(models, model => model.sync(options)).then(() => {
        // Create views
        let views = [];
        this.modelManager.forEachModel(m => {
            if (m.options.sync === false && m.createView) {
              views.push(m);
            }              
        });
        return Sequelize.Promise.each(views, v => {
          if (options.hooks) {
            return v.runHooks('createView', v, options);
          }
        // Run the views again to handle situations where a view failed because it was dependent on another. Need a better solution.
        }).then(() => {
          return Sequelize.Promise.each(views, v => {
            if (options.hooks) {
              return v.runHooks('createView', v, options);
